        ...
        for col in colls:
            ...
            query = {
                "extracted_date": {
                    ...
                    "$lt": datetime.dat        # Collect country codes for this org's db
        required_codes = set()
        db = client[db_name]
        colls = db.list_collection_names()
        for col in colls:
...
etime.combine(yesterday_date + datetime.timedelta(days=1), datetime.time.min, tzinfo=datetime.timezone.utc)
                }
            }
            for doc in db[col].find(query, {"src_geo": 1, "dst_geo": 1, "http_referer_geo": 1}):
                src = doc.get("src_geo") or doc.get("http_referer_geo")
